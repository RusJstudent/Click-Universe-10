...
import {openMenu, openMenuSection, addEquipItem, selectEquipItem, showEquipQuantity, promocodeHandler, upgradeShield, displayUpgradeInfo, showLeaderboard} from './menu.js';
...
//     const now = Date.now();
//     if (now - user.extentions.fastrep > 70000) {
//         user.extentions.fastrep = now;
//         fastrepGreen.style.display = 'block';

//         clearTimeout(repairId);

//         let repair = user.maxHp * 0.3;
...
//         animateRepair(user, repair, damageContainer, timerDamage);
//         updateHp();
//         saveData();

//         let timerRep = setInterval(() => {
//             if (user.hp < user.maxHp - repair) {
//                 user.hp += repair;
//             } else {
//                 user.hp = user.maxHp;
...
//             setTimeout(() => fastrepRed.style.display = 'none', 60000);
